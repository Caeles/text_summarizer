...
def get_langchain_summarizer():
	try:
		...
		raise RuntimeError(f"Unable to import summarizer service: {import_error}") from import_error
...
async def health():
	return {
		...
		"model": _clean_env(os.getenv("OPENAI_MODEL") or os.getenv("OPENAI_API_MODEL"), "gpt-4o-mini"),
	}
